...
<c:choose>
           ...
           <c:choose>
             <c:when test="${gvo.pay==0}">
               <c:set var="pay" value="계좌이체"/>
             </c:when>
             <c:when test="${gvo.pay==1}">
               <c:set var="pay" value="쿠페이머니"/>
             </c:when>
             <c:when test="${gvo.pay==2}">
               <c:set var="pay" value="신용/체크카드"/>
             </c:when>
             <c:when test="${gvo.pay==3}">
               <c:set var="pay" value="법인카드"/>
             </c:when>
             <c:when test="${gvo.pay==4}">
               <c:set var="pay" value="휴대폰"/>
             </c:when>
             <c:when test="${gvo.pay==5}">
   ...
   <section>
      ...
      <hr>
      <h2> 상품배송 정보</h2>
      <div id="a2"> ${pvo.bday} ${yoil2} 도착예정(상품 1개) </div>
      <div>
        <table width="900">
          <tr>
          ...
          <tr>
            ...
            <td> 주문 금액 </td>
            <td> <fmt:formatNumber value="${pvo.price*gvo.su}" pattern="#,###" type="number"/>원 </td>
          </tr>
          <tr>
            <td> 받는 주소 </td>
            <td> ${bvo.juso} ${bvo.juso_etc} </td>
            <td> 배송비 </td>
            <td> <fmt:formatNumber value="${pvo.baesong}" pattern="#,###" type="number"/>원 </td>
          </tr>
          <tr>
            <td> 배송요청사항 </td>
            <td> ${cla} </td>
            <td> 총결제금액 </td>
            <td> ${pay} <fmt:formatNumber value="${gvo.cprice}" pattern="#,###" type="number"/>원 </td>
          </tr>
        </table>
      </div>
      <div id="a4">
